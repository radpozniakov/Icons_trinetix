...
var onRun = function(context,path,fontname) {
  ...
  count             = icons.count()
  width             = 1000
  col_size          = Math.ceil(width / 78)
  row_size          = Math.ceil(count / col_size)
  height            = Math.ceil(row_size * 74)
  list              = [[NSScrollView alloc] initWithFrame:NSMakeRect(25,25,950,650)]
  ...
  matrix.setFont([NSFont fontWithName:@""+fontname size:36.0])
  matrix.setCellSize(NSMakeSize(60, 60))
  matrix.setIntercellSpacing(NSMakeSize(14, 14))
  ...
  var searchbox   = [[NSTextField alloc] initWithFrame:NSMakeRect(200,700,650,38)]
  ...
  searchbox.setCOSJSTargetFunction(function(sender) {
    ...
    newHeight           = Math.ceil(newRows * 74)
